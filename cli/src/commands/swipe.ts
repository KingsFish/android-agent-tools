...
export const definition: ToolDefinition = {
  name: 'swipe',
  description: 'Perform a swipe from start to end coordinates',
  parameters: [
  ...
  adbCommand: 'shell input swipe <start_x> <start_y> <end_x> <end_y> <duration>',
  async execute(startX: number, startY: number, endX: number, endY: number, duration?: number, options?: CliOptions): Promise<string> {
    try {
      ...
      const result = await execAdb(
        ['shell', 'input', 'swipe', String(startX), String(startY), String(endX), String(endY), String(dur)],
        { deviceId: device, timeout: options?.timeout }
      );
      return result.exitCode === 0 ? success({}) : failure(ErrorCodes.TAP_FAILED, result.stderr || 'Swipe failed');
    } catch (err: any) {
